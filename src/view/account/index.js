...
class index extends Component {
    ...
    componentDidMount() {
        ...
        this.listener = DeviceEventEmitter.addListener(TypeId.ACCOUNT_GET_SUCCESS, (param) => {  //触发关闭刷新或加载
    ...
    //切换菜单
    changeTab(obj) {
        const {menuList, menuSelectedId, dataList} = this.props.account;
        //
        this.setState({
            nowPage: obj.i
        });
    ...
    //数据列表
    _listData() {
        const {dataList, nowChildPage} = this.props.account;
        const data = [];
    ...
    _renderIndexPath = ({section: section, row: row,}) => {
        ...
        return (
            <View style={[styles.itemView, Style.rowCenterCenter]}>
                <TouchableOpacity onPress={() => this.jumpWeb(item)}>
